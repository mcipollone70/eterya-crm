...
interface SampleFiltersBarProps {
...
export function SampleFiltersBar({ agents, companies }: SampleFiltersBarProps) {
  ...
  function updateParam(key: string, value: string) {
    ...
    startTransition(() => {
      router.push(`/campioni?${params.toString()}`);
    });
  }
  ...
  const filters = (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:border-0 md:p-0 md:shadow-none">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <label className="block text-sm">
          <span className="mb-1 block text-xs font-medium text-slate-600">Consegna da</span>
          <input
            type="date"
        ...
        <label className="block text-sm">
          <span className="mb-1 block text-xs font-medium text-slate-600">Consegna a</span>
          <input
            type="date"
