...
function buildCompaniesQuery(
  ...
  updates: { page?: number; pageSize?: CompaniesPageSize }
): string {
  const params = new URLSearchParams(searchParams.toString());

  if (updates.pageSize !== undefined) {
    if (updates.pageSize === parseCompaniesPageSize(undefined)) {
  ...
  if (updates.page !== undefined) {
    if (updates.page <= COMPANIES_DEFAULT_PAGE) {
      params.delete("page");
    } else {
...
export function CompaniesPagination({ total, page, pageSize }: CompaniesPaginationProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const safePage = clampCompaniesPage(page, total, pageSize);
  const totalPages = getCompaniesTotalPages(total, pageSize);
  const rangeLabel = formatCompaniesVisibleRange(safePage, pageSize, total);

  function navigate(nextPage: number) {
    router.push(buildCompaniesQuery(searchParams, { page: nextPage }));
  }

  function handlePageSizeChange(event: React.ChangeEvent<HTMLSelectElement>) {
    const nextSize = Number.parseInt(event.target.value, 10) as CompaniesPageSize;
    router.push(buildCompaniesQuery(searchParams, { pageSize: nextSize, page: COMPANIES_DEFAULT_PAGE }));
  }
    ...
    <div className="flex flex-col gap-3 border-t border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <label className="flex items-center gap-2 text-sm text-slate-600">
          ...
          >
            {COMPANIES_PAGE_SIZE_OPTIONS.map((option) => (
