    ...
    <div className="mx-auto max-w-3xl bg-white p-8 text-slate-900 print:p-0">
      <div className="mb-8 flex items-start justify-between gap-4 border-b border-slate-300 pb-6">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-bold">Ordine</h1>
          <p className="mt-1 text-sm text-slate-600">{order.number ?? "Senza numero"}</p>
        </div>
        <div className="text-right text-sm">
          <p className="font-medium">{statusLabel}</p>
          <p className="text-slate-500">
            Data: {formatVisitDate(order.order_date ?? order.accepted_at ?? order.created_at)}
          </p>
          <p className="text-slate-500">
            Consegna: {formatVisitDate(order.expected_delivery_at)}
          </p>
        </div>
      </div>
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-1 text-lg font-semibold">{order.company_name ?? "—"}</p>
          {order.contact_name && (
            <p className="text-sm text-slate-600">Referente: {order.contact_name}</p>
          )}
        </div>
        <div>
          ...
          <p className="text-sm text-slate-600">
            {PRODUCT_FAMILY_LABELS[order.product_family]}
          </p>
        </div>
      </div>
